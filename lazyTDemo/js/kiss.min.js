...
if(deep&&src&&(S.isArray(src)||S.isPlainObject(src))){var clone=target&&(S.isArray(target)||S.isPlainObject(target))?target:(S.isArray(src)?[]:{});r[p]=S.mix(clone,src,ov,undefined,true);}else if(src!==undefined){r[p]=s[p];}}},seed=(host&&host[S])||{},guid=0,EMPTY='';host=seed.__HOST||(seed.__HOST=host||{});S=host[S]=meta.mix(seed,meta);S.mix(S,{configs:{},__APP_MEMBERS:['namespace'],__APP_INIT_METHODS:['__init'],version:'1.20',buildTime:'20120208172832',merge:function(){var o={},i,l=arguments.length;for(i=0;i<l;i++){S.mix(o,arguments[i]);}
...
var bufferTimer=null;function f(){f.stop();bufferTimer=S.later(fn,ms,FALSE,context||this);}
...
function cssPoll(){for(var url in monitors){var callbacks=monitors[url],node=callbacks.node,loaded=0;if(utils.isWebKit){if(node['sheet']){S.log("webkit loaded : "+url);loaded=1;}}else if(node['sheet']){try{var cssRules;if(cssRules=node['sheet'].cssRules){S.log('firefox  '+cssRules+' loaded : '+url);loaded=1;}}catch(ex){if(ex.code===1000){S.log('firefox  '+ex.name+' loaded : '+url);loaded=1;}}}
...
var oldCallback=node.onreadystatechange;node.onreadystatechange=function(){var rs=node.readyState;if(/loaded|complete/i.test(rs)){node.onreadystatechange=null;oldCallback&&oldCallback();callback.call(this);}};},styleOnload:window.attachEvent?function(node,callback){function t(){node.detachEvent('onload',t);S.log('ie/opera loaded : '+node.href);callback.call(node);}
...
function build(fullpath,path){if(!mod[fullpath]&&mod[path]){mod[path]=utils.normalDepModuleName(mod.name,mod[path]);mod[fullpath]=base+mod[path];}
...
var packages=self.Config.packages;for(var p in packages){if(packages.hasOwnProperty(p)){var p_path=packages[p].path;if(packages.hasOwnProperty(p)&&src.lastIndexOf(p_path,0)===0){return utils.removePostfix(src.substring(p_path.length));}}}
...
var ATTACHED=data.ATTACHED,mix=S.mix;mix(loader,{__pagePath:location.href.replace(location.hash,"").replace(/[^/]*$/i,""),__currentModule:null,__startLoadTime:0,__startLoadModuleName:null,__isAttached:function(modNames){var mods=this.Env.mods,ret=true;S.each(modNames,function(name){var mod=mods[name];if(!mod||mod.status!==ATTACHED){ret=false;return ret;}});return ret;}});})(KISSY,KISSY.__loader,KISSY.__loaderData);(function(S,loader,utils){if("require"in this){return;}
...
var self=this,modName=S.__getCombinedMod(mod.name),packages=self.Config.packages||{},pName="",p_def;for(var p in packages){if(packages.hasOwnProperty(p)){if(S.startsWith(modName,p)&&p.length>pName){pName=p;}}}
...
var retryCount=1,node,timer=S.later(function(){if((node=doc.getElementById(id))&&(fn(node)||1)||++retryCount>POLL_RETRYS){timer.cancel();}},POLL_INTERVAL,true);}});function _bindReady(){var doScroll=docElem.doScroll,eventType=doScroll?'onreadystatechange':'DOMContentLoaded',COMPLETE='complete',fire=function(){_fireReady();};if(doc.readyState===COMPLETE){return fire();}
...
_bindReady();})(KISSY,undefined);(function(S){S.config({'combines':{'core':['dom','ua','event','node','json','ajax','anim','base','cookie']}});})(KISSY);KISSY.add('ua/base',function(){var ua=navigator.userAgent,EMPTY='',MOBILE='mobile',core=EMPTY,shell=EMPTY,m,IE_DETECT_RANGE=[6,9],v,end,VERSION_PLACEHOLDER='{{version}}',IE_DETECT_TPL='',div=document.createElement('div'),s,o={},numberify=function(s){var c=0;return parseFloat(s.replace(/\./g,function(){return(c++===0)?'.':'';}));};div.innerHTML=IE_DETECT_TPL.replace(VERSION_PLACEHOLDER,'');s=div.getElementsByTagName('s');if(s.length>0){shell='ie';o[core='trident']=0.1;if((m=ua.match(/Trident\/([\d.]*)/))&&m[1]){o[core]=numberify(m[1]);}
...
timer=setTimeout(poll,POLL_INTERVAL);},hashChange=ie&&ie<8?function(hash){var html=S.substitute(IFRAME_TEMPLATE,{hash:hash,head:DOM._isCustomDomain()?"<script>document.domain = '"+
doc.domain
...
xhrObj.responseText=xhr.responseText;try{var statusText=xhr.statusText;}catch(e){S.log("xhr statustext error : ");S.log(e);statusText="";}
...
DOM.attr(form,this.attrs);if(eventType=="load"){var iframeDoc=iframe.contentWindow.document;xhr.responseXML=iframeDoc;xhr.responseText=DOM.text(iframeDoc.body);xhr.callback(OK_CODE,"success");}else if(eventType=='error'){xhr.callback(ERROR_CODE,"error");}
...
return s;}
function setInternal(self,name,value,opts,attrs){var ret;opts=opts||{};var dot=".",path,subVal,prevVal,fullName=name;if(name.indexOf(dot)!==-1){path=name.split(dot);name=path.shift();}
...
getAttrVals(self)[name]=value;},get:function(name){var self=this,dot=".",path,attrConfig,getter,ret;if(name.indexOf(dot)!==-1){path=name.split(dot);name=path.shift();}
